...
doc <- paste0("Usage: install2.r [-l LIBLOC] [-h] [-x] [-s] [-p NTRY] [-d DEPS] [-n NCPUS] [-r REPOS...] [-m METHOD] [-t TYPE] [--error] [--] [PACKAGES ...]
...
-p --ntry NTRY			the number of (re)tries in case something fails [default: 1]
...
opt$ntry<-as.numeric(opt$ntry)


## ensure installation is stripped
Sys.setenv("_R_SHLIB_STRIP_"="true")

install_packages2 <- function(pkgs, ..., error = FALSE, skipinstalled = FALSE, ntry = 1) {
    e <- NULL
    capture <- function(e) {
        if (error) {
            catch <-
